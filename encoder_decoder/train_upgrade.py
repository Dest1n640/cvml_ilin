...
class Encoder(nn.Module):
  def __init__(self, latent_size=1024):
    ...
    self.bottleneck = nn.Linear(1024 * 4 * 4, latent_size)
...
class Decoder(nn.Module):
  def __init__(self, latent_size=1024):
    super().__init__()
    self.bottleneck = nn.Linear(latent_size, 1024 * 4 * 4)
    self.features = nn.Sequential(
...
def train_model(mode, device):
  ...
  torch.save(encoder.state_dict(), Path(f"tmp/encoder_upgrade{mode}.pth"))
  torch.save(decoder.state_dict(), Path(f"tmp/decoder_upgrade{mode}.pth"))
